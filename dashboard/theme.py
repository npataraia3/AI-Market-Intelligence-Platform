...
.sidebar-brand:hover {{ background: var(--surface-alt); }}
.sidebar-brand-title {{
    font-size: 11px;
    font-weight: 700;
    letter-spacing: .14em;
...
.nav-item.nav-home:hover {{ background: var(--accent-bg); }}
.nav-item.nav-home.active {{ background: var(--accent-bg); border-color: var(--accent); }}
.home-nav-separator {{
    height: 1px;
...
.home-nav-item-active {{ color: var(--accent); background: var(--accent-bg); }}
.home-nav-item-active:hover {{ color: var(--accent); background: var(--accent-bg); }}
...
@media (max-width: 900px) {{
    ...
    .dash-app-shell.sidebar-open .sidebar {{ transform: translateX(0); box-shadow: 24px 0 48px rgba(20,24,23,.2); }}
    .main {{ width: 100%; }}
    .topbar {{ padding: 10px 16px; gap: 12px; }}
    .page {{ padding: 18px 16px 40px; }}
    .topbar-page {{ display: none; }}
    #active-coin-label {{ display: none; }}
}}
