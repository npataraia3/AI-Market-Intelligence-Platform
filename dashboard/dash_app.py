    ...
    <body>
        ...
        (function () {
            ...
            function closeSidebar() {
                var shell = document.getElementById('dash-app-shell');
                if (shell) shell.classList.remove('sidebar-open');
            }
            document.addEventListener('click', function (event) {
                var toggle = event.target && event.target.closest ? event.target.closest('#nav-toggle') : null;
                if (toggle) {
                    var shell = document.getElementById('dash-app-shell');
                    if (shell) shell.classList.toggle('sidebar-open');
                    return;
                }
                var navLink = event.target && event.target.closest ? event.target.closest('.sidebar a') : null;
                if (navLink) { closeSidebar(); return; }
...
nav_children = [
    dcc.Link("Home", id="nav-home", href="/", className="nav-item nav-home"),
]
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                ...
                                dcc.Link("← Back to home", href="/", className="topbar-home"),
...
def router(pathname: str):
    ...
    nav_home_class = "nav-item nav-home active" if active is None else "nav-item nav-home"
    return [home_style, shell_style, *page_styles, page_label, *nav_classes, nav_home_class]
