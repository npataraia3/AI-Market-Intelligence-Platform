...
def build_frame(history: dict, fear_greed: list[dict] | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Public wrapper around the feature-engineering pipeline."""
    return _build_features(history, fear_greed or [])
...
def _build_features(history: dict, fear_greed: list[dict]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    rsi = (100 - 100 / (1 + gains / losses.replace(0, np.nan))).fillna(50).clip(0, 100)
    # Losses of exactly zero mean an unbroken up-leg: that is RSI 100, not neutral.
    strong_uptrend = (losses == 0) & (gains > 0)
    frame["rsi_14"] = rsi.mask(strong_uptrend, 100.0)
...
def _directional_accuracy(prediction: np.ndarray, price_level: np.ndarray, actual: np.ndarray) -> float:
    """Fraction of steps where the predicted move direction matches the realized one."""
    predicted_direction = np.sign(prediction - price_level)
    actual_direction = np.sign(actual - price_level)
    correct = predicted_direction * actual_direction > 0
    return float(correct.mean()) if len(correct) else 0.0


def _classification_metrics(frame: pd.DataFrame) -> dict[str, float] | None:
    """Directional task (up/down) evaluated with an XGBoost classifier and a
    naive "always positive" benchmark, using the same leakage-free folds."""
    ...
    if len(train) < 50:
        return None
    target = (train["next_return"] > 0).astype(int)
    if target.nunique() < 2:
        return None
    ...
    for train_index, test_index in splitter.split(train):
        ...
        model = XGBClassifier(n_estimators=120, max_depth=3, learning_rate=0.05, subsample=0.9, random_state=42, n_jobs=1)
        model.fit(train_part[FEATURE_COLUMNS], y_train)
        probs = model.predict_proba(test_part[FEATURE_COLUMNS])[:, 1]
    ...
    averaged = {key: float(np.mean([row[key] for row in rows if row[key] is not None])) for key in rows[0] if key != "always_positive_accuracy"}
    averaged["always_positive_accuracy"] = float(np.mean([row["always_positive_accuracy"] for row in rows]))
    return averaged
...
def _rolling_evaluation(frame: pd.DataFrame, arima_order: tuple[int, int, int]) -> tuple[list[dict], pd.DataFrame]:
    ...
    directional_scores: dict[str, list[float]] = {name: [] for name in results}
    ...
    for train_index, test_index in splitter.split(train):
        ...
        for model_name, prediction in predictions.items():
            ...
            directional = _directional_accuracy(np.asarray(prediction), test_part["price_usd"].to_numpy(), actual.to_numpy())
            results[model_name].append((score["mae"], score["rmse"], str(test_part["timestamp"].iloc[-1])))
            directional_scores[model_name].append(directional)
    ...
    for model_name, values in results.items():
        ...
        comparison.append({
            "model": model_name, "mae": round(float(np.mean(mae_values)), 4),
            "rmse": round(float(np.mean(rmse_values)), 4), "folds": len(values),
            "directional_accuracy": round(float(np.mean(directional_scores[model_name])), 4),
        })
...
def fit_explainer_model(frame: pd.DataFrame, selected_model: str):
    """Fit the same ML model used for forecasting, returning (model, features)
    so external services can attach SHAP explanations without refactoring."""
    train = frame.dropna(subset=FEATURE_COLUMNS + ["next_return"])
    model = _ml_model(selected_model)
    model.fit(train[FEATURE_COLUMNS], train["next_return"])
    return model, train[FEATURE_COLUMNS].reset_index(drop=True)
